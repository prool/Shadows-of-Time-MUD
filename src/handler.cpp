...
void update_room args((char *name, int count));
void check_trap( CHAR_DATA *ch, OBJ_DATA *obj );

bool IS_ASSMASTER(CHAR_DATA *ch, CHAR_DATA *victim)
{
  if (IS_AFFECTED(victim, AFF_DIEMCLOAK) && !str_cmp(ch->name, "Asmodeus"))
    return TRUE;
  return FALSE;
}

/* guild stuff -- for finding if char is in a certain guild*/
bool is_guild (CHAR_DATA *ch, char *argument)
{
    if (IS_NPC(ch))
        return FALSE;
        
    if (ch->clan == clan_lookup("argument"))
        return TRUE;
    else
        return FALSE;
}
...
/*returns true value if obj is out of character's level*/

bool is_obj_classrestrict (CHAR_DATA * ch,OBJ_DATA * obj)
{
  int flag;
  ...
  if ((flag = flag_value(class_flags, class_table[ch->cClass].name)))
  {
    if (!IS_SET(obj->class_flags, flag))
      return TRUE; 
  }
  return FALSE;
}

bool is_obj_outlevel (CHAR_DATA * ch,OBJ_DATA * obj)
{
	int variance;
    int level;
 
    level = IS_FORSAKEN(ch) ? ch->level + 1 : IS_DRAGON(ch) ? ch->level + 1 : ch->level;

    if (level >= (MAX_LEVEL-5))
      variance = -1;
    else 
      variance = (2 * level / 5) + 5; 
  
    if ((obj->level - level > variance) && (variance != -1))
        return TRUE;
    else
 	return FALSE;
}
...
/*Counts the active people/characters in the room - Used for RPexp - CAMM*/
/*additional checks for sleeping and afk chars*/
int count_people (CHAR_DATA * ch)
{
	ROOM_INDEX_DATA *location;
	int count = 0;
	location = ch->in_room;
	for (CHAR_DATA *wch = location->people; wch; wch = wch->next_in_room)
        {
          if (!IS_NPC (wch)
			&& wch->position != POS_SLEEPING
			&& !IS_SET (wch->comm, COMM_AFK)
			&& wch->timer < 10 && can_see(ch, wch))
		{
			count++;
        }
    }
...
char *explosive_name (int explosive_type)
{
    int type;

    for (type = 0; explosive_table[type].name != NULL; type++)
        if (explosive_type == explosive_table[type].type)
            return explosive_table[type].name;
    return "none";
}
...
int class_lookup (const char *name)
{
    ...
    {
        if (LOWER (name[0]) == LOWER (class_table[cClass].name[0])
            && !str_prefix (name, class_table[cClass].name))
            return cClass;
    }
    ...
    {                            /* mobiles */
        ...
        if (skill_table[sn].spell_fun != spell_null)
            ...
            skill = 40 + ch->level;
                 ...
                 || sn == gsn_mace
                 || sn == gsn_axe
                 || sn == gsn_flail || sn == gsn_whip || sn == gsn_polearm || sn == gsn_bow)
...
int get_weapon_skill (CHAR_DATA * ch, int sn)
{
    ...
    if (IS_NPC (ch))
    {
        if (sn == -1)
            skill = 3 * ch->level;
        else if (sn == gsn_hand_to_hand ||sn == gsn_martial_arts)
            skill = 40 + 2 * ch->level;
        else
            skill = 40 + 5 * ch->level / 2;
    }
...
void reset_char (CHAR_DATA * ch)
{
            ...
            for (af = obj->pIndexData->affected; af != NULL; af = af->next)
            {
                ...
                switch (af->location)
                {
                    ...
                    case APPLY_SAVING_SPELL:
                        ch->saving_throw += mod;
                        break;
                    case APPLY_SPELL_STR:
                        ch->mod_stat[STAT_STR] += mod;
                        break;
                    case APPLY_SPELL_DEX:
                        ch->mod_stat[STAT_DEX] += mod;
                        break;
                    case APPLY_SPELL_INT:
                        ch->mod_stat[STAT_INT] += mod;
                        break;
                    case APPLY_SPELL_WIS:
                        ch->mod_stat[STAT_WIS] += mod;
                        break;
                    case APPLY_SPELL_CON:
                        ch->mod_stat[STAT_CON] += mod;
                        break;

                    case APPLY_SPELL_SEX:
                        ch->sex += mod;
                        break;
                    case APPLY_SPELL_MANA:
                        ch->max_mana += mod;
                        break;
                    case APPLY_SPELL_HIT:
                        ch->max_hit += mod;
                        break;
                    case APPLY_SPELL_MOVE:
                        ch->max_move += mod;
                        break;
                    ...
                    case APPLY_SPELL_HITROLL:
                        ch->hitroll += mod;
                        break;
                    case APPLY_SPELL_DAMROLL:
                        ch->damroll += mod;
                        break;

                    case APPLY_SPELL_SAVES:
                        ch->saving_throw += mod;
                        break;
                    case APPLY_SPELL_SAVING_ROD:
                        ch->saving_throw += mod;
                        break;
                    case APPLY_SPELL_SAVING_PETRI:
                        ch->saving_throw += mod;
                        break;
                    case APPLY_SPELL_SAVING_BREATH:
                        ch->saving_throw += mod;
                        break;
                    case APPLY_SPELL_SAVING_SPELL:
                        ch->saving_throw += mod;
                        break;
                }
            }
    ...
    { /* Add frighten */  
      int sn1, sn2;
      if ((sn1 = skill_lookup("fear")) == -1)
         return;
      if ((sn2 = skill_lookup("frighten")) == -1)
         return;
      if (ch->pcdata->learned[sn1] > 0 && ch->pcdata->learned[sn2] < 2)
        ch->pcdata->learned[sn2] = ch->pcdata->learned[sn1];
    }
    ...
    { /* Check for miscouple */
      ...
      {
        sn1 = ch->pcdata->couple[i].cast;   
        sn2 = ch->pcdata->couple[i].couple;   
        if (ch->pcdata->learned[sn1] < 1 || ch->pcdata->learned[sn2] < 1)
        {
...
{
    int max;
    max = pc_race_table[ch->race].modstats[stat] + class_table[ch->cClass].modstat[stat];
...
int get_curr_stat (CHAR_DATA * ch, int stat)
{
   ...
   int temp = 0;
   max = 25;
   if (ch->mod_stat[stat] >= 0)    
     return URANGE (18, ch->perm_stat[stat], max);
   else
     temp = URANGE (18 + ch->mod_stat[stat], ch->perm_stat[stat] + ch->mod_stat[stat] , max);
   if (temp < 3)
     temp = 3;
   return temp;
}

int get_weave_stat (CHAR_DATA *ch, int stat)
{
  int sum;
  if (IS_NPC(ch))
    return 8;

  sum = class_table[ch->cClass].weaves[stat];
  sum += sex_table[ch->sex].weaves[stat];
  sum += ch->pcdata->weaves[stat];
  return sum;
} 
...
int get_max_train (CHAR_DATA * ch, int stat)
{
    ...
    if (class_table[ch->cClass].attr_prime == stat)
    {
...
void affect_modify (CHAR_DATA * ch, AFFECT_DATA * paf, bool fAdd)
{
    ...
    if (fAdd)
    {
        ...
        //        STR_SET_BIT (ch->affected, paf->bitvector);
                break;
           case TO_IMMUNE:
    ...
    else
    {
        ...
        //    case TO_SIMAFF:
        //        REMOVE_BIT (ch->simaff, paf->bitvector);
        //        break;
           case TO_IMMUNE:
    ...
    switch (paf->location)
    {
        ...
        case APPLY_SPELL_SEX:
            ch->sex += mod;
            break;
        case APPLY_SPELL_CLASS:
            break;
        case APPLY_SPELL_LEVEL:
            break;
        case APPLY_SPELL_AGE:
            break;
        case APPLY_SPELL_HEIGHT:
            break;
        case APPLY_SPELL_WEIGHT:
            break;
        case APPLY_SPELL_MANA:
            ch->max_mana += mod;
            break;
        ...
        (str_app[get_curr_stat (ch, STAT_STR)].wield * 10))
    {
        ...
        return; // No drop of weapons while noloot
        if (depth == 0)
        {
            depth++;
            ...
            {
              act ("You are to weak to wield $p anymore you drop it into your inventory.", ch, wield, NULL, TO_CHAR);
              act ("$n stops wielding $p.", ch, wield, NULL, TO_ROOM);
              obj_from_char (wield);
...
void rem_obj_affect (CHAR_DATA *ch, int bitvector)
{
  AFFECT_DATA *paf;
...
void add_obj_affect (CHAR_DATA * ch, int bitvector)
{
    AFFECT_DATA af;
    AFFECT_DATA *paf;
    int sn;

    if ((sn = skill_lookup(affect_flags[bitvector -1].name)) < 1)
      return; 
 ...
 */
void affect_strip_bv (CHAR_DATA * ch, int sn)
{
    AFFECT_DATA *paf;
    AFFECT_DATA *paf_next;
...
void char_to_room (CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex)
{
    ...
    if (!IS_NPC (ch))
    {
        if (!getbit(ch->pcdata->explored, ch->in_room->vnum))
        {
          ...
          if (!IS_SET(ch->act2, PLR_NOEXP))
            gain_exp(ch, number_range(2, 5));
        }
...
/*
 * Strip all affects of a given bitvector && perm.
 */
void obj_affect_strip (CHAR_DATA * ch, int affect)
{
    AFFECT_DATA *paf;
    AFFECT_DATA *paf_next;
    ...
    {
        paf_next = paf->next;
        if (paf->bitvector == affect && paf->duration == -1 && paf->type == 0)
            affect_remove (ch, paf);
    }
...
void equip_char (CHAR_DATA * ch, OBJ_DATA * obj, int iWear)
{
    ...
    {
        logf ("Equip_char: already equipped (%d) %s room %s.", 
		iWear, ch->short_descr, ch->in_room->name);
        return;
    }
...
*/
    ...
    if (!obj->enchanted)
    {
      for (paf = obj->pIndexData->affected; paf != NULL; paf = paf->next)
      { 
           if (!IS_SPELL_AFFECT(paf->location))
           {
             affect_modify (ch, paf, TRUE);
           }
           if (paf->bitvector && !IS_NPC(ch))
           {
         //    add_obj_affect(ch, paf->bitvector);
           }
      }
    }
    ...
    for (paf = obj->affected; paf != NULL; paf = paf->next)
        if (IS_SPELL_AFFECT(paf->location))
        {
            npaf.where = TO_AFFECTS;
            npaf.type= paf->type;
            npaf.level = paf->level;
            npaf.duration = paf->duration;
            npaf.bitvector = paf->bitvector;
            if (paf->bitvector == AFF_BERSERK)
            {
                npaf.modifier = UMAX (1, paf->level / 5);    
                npaf.location = APPLY_SPELL_HITROLL;
                affect_to_char (ch, &npaf);
                
                npaf.location = APPLY_SPELL_DAMROLL;
                affect_to_char (ch, &npaf);
                
                npaf.modifier = UMAX (10, 10 * (paf->level / 5));
                npaf.location = APPLY_SPELL_AC;
                affect_to_char (ch, &npaf);
                continue;
            }
            if (paf->bitvector == AFF_BLESS)
            {     
                npaf.modifier = paf->level / 8;            
                npaf.location = APPLY_SPELL_HITROLL;
                affect_to_char (ch, &npaf);
                
                npaf.modifier = 0 - paf->level / 8;
                npaf.location = APPLY_SPELL_SAVES;
                affect_to_char (ch, &npaf);
                
                continue;
            }
            if (paf->bitvector == AFF_FRENZY)
            {
                npaf.modifier = paf->level / 6;         
                npaf.location = APPLY_SPELL_HITROLL;
                affect_to_char (ch, &npaf);
            ...
            if (paf->bitvector == AFF_STONESKIN)
            {
            ...
            if (paf->bitvector == AFF_WEAKEN)
            {
                npaf.modifier = -1 * (paf->level / 5);         
                npaf.location = APPLY_SPELL_STR;
                affect_to_char (ch, &npaf);
                
                continue;
            }
            if (paf->bitvector == AFF_RIFT)
            {
                npaf.modifier = -1;
                npaf.location = APPLY_SPELL_STR;
                affect_to_char (ch, &npaf);
            ...
            if (paf->bitvector == AFF_SLOW)
            {
                npaf.modifier = -1 - (paf->level >= 18) - (paf->level >= 25) - (paf->level >= 32);
                npaf.location = APPLY_SPELL_DEX;
                affect_to_char (ch, &npaf);
                
                continue;
            }       
            if (paf->bitvector == AFF_FAERIE_FIRE)
            {
                npaf.modifier = 2*paf->level;
                npaf.location = APPLY_SPELL_DEX;
                affect_to_char (ch, &npaf);
                
                continue;
            }    
            if (paf->bitvector == AFF_HASTE)
            {
                npaf.modifier = 1 + (paf->level >= 18) + (paf->level >= 25) + (paf->level >= 32);
                npaf.location = APPLY_SPELL_DEX;
                affect_to_char (ch, &npaf);
...
void unequip_char (CHAR_DATA * ch, OBJ_DATA * obj)
{
    ...
    if (location == WEAR_WAIST)
    {
      OBJ_DATA *keyring;
      if ((keyring = get_eq_char(ch, WEAR_ON_BELT)) != NULL)
      {
        unequip_char(ch, keyring);
      }
    }
    ...
    if (!obj->enchanted)
    {
        for (paf = obj->pIndexData->affected; paf != NULL; paf = paf->next)
        {
            ...
            else
            {
                ...
                if (paf->bitvector)
                  rem_obj_affect(ch, paf->bitvector);
            }
        }
    }
...
void extract_obj (OBJ_DATA * obj)
{
    ...
    /*if (obj->item_type==ITEM_CORPSE_PC)
    	{
            del_corpse (get_corpse_data(obj));
            save_corpses ();
    	}*/
...
CHAR_DATA *get_char_room (CHAR_DATA * ch, ROOM_INDEX_DATA *room, char *argument)
{
    ...
    {
        if (ch && (!can_see (ch, rch) || ((!is_name (arg, GET_CHAR_PERS(rch, ch, FALSE)) &&
            (!IS_NPC(rch) || !is_name(arg, rch->name)) ) &&
            (arg[0] != '\0')) ))
            continue;

        if (!ch && !is_name( arg, rch->name ) )
	    continue;
  ...
  for ( ach = char_list; ach != NULL ; ach = ach->next )
    {
      if ( ach->in_room->area != ch->in_room->area
          || !can_see( ch, ach ) || 
          (!is_name( arg, GET_CHAR_PERS(ach, ch, FALSE) ) 
            && (!IS_NPC(ach) || !is_name(arg, ach->name)) ) )
        continue;
      if ( ++count == number )
        return ach;
    }
...
bool get_char_exists (CHAR_DATA *ch)
{
  CHAR_DATA *wch;
  for (wch = char_list; wch != NULL; wch = wch->next)
  {
...
CHAR_DATA *get_char_world (CHAR_DATA * ch, char *argument)
{
    ...
    for (d = descriptor_list; d != NULL; d = d->next)
    {
      ...
      if (ch && (wch->in_room == NULL || !can_see (ch, wch)
          || !is_name (arg, GET_CHAR_PERS(wch, ch, FALSE))))
          continue;

      if (!ch && (wch->in_room == NULL || !is_name(arg, wch->name)))
        continue;
    ...
    for (wch = char_list; wch != NULL; wch = wch->next)
    {
       ...
       if (ch && (wch->in_room == NULL || !can_see (ch, wch)
            || (!is_name (arg, GET_CHAR_PERS(wch, ch, FALSE))
            && (!IS_NPC(wch) || !is_name(arg, wch->name)) ) ))
            continue;
...
CHAR_DATA *get_char_world_special (char *argument)
{
//    char arg[MAX_INPUT_LENGTH];
    CHAR_DATA *wch;
    int number;
...
CHAR_DATA *get_char_world_vnum (CHAR_DATA *ch, int vnum)
{
    CHAR_DATA *wch;

    for (wch = char_list; wch != NULL; wch = wch->next)
    {
        if (!IS_NPC(wch) || wch->pIndexData->vnum != vnum || !can_see(ch, wch))
            continue;
        return wch;
    }
...
CHAR_DATA *char_here (CHAR_DATA *ch)
{
   DESCRIPTOR_DATA *d;

    for (d = descriptor_list; d != NULL; d = d->next)
    {
       if ((d->character == ch || d->original == ch) && d->connected == CON_PLAYING)
         return ch; 
    }
    return NULL;
}
...
OBJ_DATA *get_obj_list_full (CHAR_DATA *ch, char *argument, OBJ_DATA * list)
{
    char arg[MAX_INPUT_LENGTH];
    OBJ_DATA *obj;
    ...
    for (obj = list; obj != NULL; obj = obj->next_content)
    {
        ...
        {
          OBJ_DATA *tmp;
          if ((tmp = get_obj_list(ch, argument, obj->contains)) != NULL)
            return tmp;
        }
    }
 ...
 * Find an obj in a list.
 */
OBJ_DATA *get_portal (CHAR_DATA * ch, OBJ_DATA * list, int vnum)
{
    OBJ_DATA *obj;

    for (obj = list; obj != NULL; obj = obj->next_content)
    {
        if (can_see_obj (ch, obj) && obj->item_type == ITEM_PORTAL &&
	    obj->value[3] == vnum)
        {
           return obj;
        }
...
int char_has(CHAR_DATA *ch, OBJ_DATA *suspect)
{
    OBJ_DATA *obj;
    OBJ_DATA *in_obj;
    int number;
    ...
    for ( obj = object_list; obj != NULL; obj = obj->next )
    {
        ...
        if (obj->pIndexData != suspect->pIndexData )
         continue;
...
int char_has_carry(CHAR_DATA *ch, OBJ_DATA *suspect)
{
    OBJ_DATA *obj;
    int number;
    number = 0;
...
OBJ_DATA *get_obj_carry (CHAR_DATA * ch, char *argument, CHAR_DATA * viewer)
{
	...
	&& (can_see_obj (viewer, obj))
        && ( (is_name (arg, obj->name)) || (ch && arg[0] == '\0')) )
        {
            if (++count == number)
                return obj;
        }
 ...
 * Create a 'money' obj.
 */
OBJ_DATA *create_money (long gold, long silver)
{
...
int get_obj_number (OBJ_DATA * obj)
{
    ...
    if ( obj->item_type == ITEM_MONEY
        || obj->item_type == ITEM_GEM || obj->item_type == ITEM_JEWELRY || obj->item_type == ITEM_TRASH)
        number = 0;
    else
        number = 1;
...
*/
    ...
    if (
       (!IS_NPC(ch) && (IS_SET (pRoomIndex->room_flags, ROOM_HEROES_ONLY)))
         && (((ch->level < 76) && !IS_FORSAKEN(ch) && !IS_DRAGON(ch)) || 
           (ch->pcdata->pk_timer != 0 &&  
	   !IS_SET(ch->in_room->room_flags, ROOM_HEROES_ONLY))) )
        return FALSE;

    if (IS_SET (pRoomIndex->room_flags, ROOM_NEWBIES_ONLY)
        && ch->level > 10 && !IS_IMMORTAL (ch) && ch->clan != clan_lookup("Guide"))
        return FALSE;
...
/* visibility on a room -- for entering and exits */
bool room_is_wall (CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex)
{
   
	if (pRoomIndex->sector_type == SECT_WALL)
		return FALSE;
...
/* visibility on a room -- for entering and exits */
bool room_is_gate (CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex)
{
	if (pRoomIndex->sector_type == SECT_GATE)
	{
		if ( IS_SET (pRoomIndex->room_flags, ROOM_AIEL) && 
			(ch->clan != clan_lookup("Aiel") ) )
			return FALSE;
		if ( IS_SET (pRoomIndex->room_flags, ROOM_SHADOW) && 
			(ch->clan != clan_lookup("Shadow") ) )
			return FALSE;
		if ( IS_SET (pRoomIndex->room_flags, ROOM_WHITETOWER) && 
			(ch->clan != clan_lookup("Warder") ) )
			return FALSE;
		if ( IS_SET (pRoomIndex->room_flags, ROOM_WHITETOWER) && 
			(ch->clan != clan_lookup("Aessedai") ) )
			return FALSE;
		if ( IS_SET (pRoomIndex->room_flags, ROOM_WHITECLOAK) && 
			(ch->clan != clan_lookup("Whitecloak") ) )
			return FALSE;
		if ( IS_SET (pRoomIndex->room_flags, ROOM_ASHAMAN) && 
			(ch->clan != clan_lookup("Ashaman") ) )
			return FALSE;
		if ( IS_SET (pRoomIndex->room_flags, ROOM_SHAIDO) && 
...
/* visibility on a room -- for entering and exits */
bool room_is_tower (CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex)
{
	if (pRoomIndex->sector_type == SECT_TOWER)
		return FALSE;

    return TRUE;
}

bool check_invis (CHAR_DATA *ch, CHAR_DATA *victim)
{
  if (ch->clan == clan_lookup("Wolfbrother") && !furies_imm(ch, victim))
    return TRUE;
...
bool check_hidden (CHAR_DATA *ch, CHAR_DATA *victim)
{
  if (ch->clan == clan_lookup("Wolfbrother") && !furies_imm(ch, victim))
    return TRUE;
...
bool check_shroud (CHAR_DATA *ch, CHAR_DATA *victim)
{
  // No need to worry about furies shrouding
  if (ch->clan == victim->clan) /*|| ch->clan == clan_lookup("whitecloak"))*/
    return TRUE;
  
  if (IS_NPC(ch) && IS_AFFECTED(ch, AFF_DETECT_HIDDEN))
    return TRUE;

  if (furies_imm(victim, ch))
    return TRUE;
...
bool can_see (CHAR_DATA * ch, CHAR_DATA * victim)
{
    ...
    if (ch == victim)
      return TRUE;
    if (IS_ASSMASTER(ch, victim))
      return TRUE;

    if (IS_AFFECTED(victim, AFF_DIEMCLOAK))
      return FALSE;
    if (IS_AFFECTED(victim, AFF_SUPER_INVIS)) // For Load Time
    {
    ...
    if (IS_SET(ch->act2, PLR_DREAM) 
    && !IS_SET(victim->act2, PLR_DREAM))
        return FALSE;
    ...
    if (
         ( !IS_AFFECTED(victim, AFF_INVISIBLE) || check_invis(ch, victim)  ) &&
         ( !IS_AFFECTED(victim, AFF_HIDE)      || check_hidden(ch, victim) ) &&
         ( !IS_AFFECTED(victim, AFF_SHROUD)    || check_shroud(ch, victim) ) 
        )
        return TRUE;
...
bool can_see_ooc (CHAR_DATA * ch, CHAR_DATA * victim)
{
    ...
    if (
         ( !IS_AFFECTED(victim, AFF_INVISIBLE) || check_invis(ch, victim)  ) &&
         ( !IS_AFFECTED(victim, AFF_HIDE)      || check_hidden(ch, victim) ) &&
         ( !IS_AFFECTED(victim, AFF_SHROUD) || ch->clan == victim->clan ) 
        )
        return TRUE;
...
bool can_see_obj (CHAR_DATA * ch, OBJ_DATA * obj)
{
    ...
    if (IS_SET (obj->extra_flags, ITEM_INVIS)
        ...
        && (ch->clan != clan_lookup("Wolfbrother")) )
        return FALSE;
...
char *affect_bit_name (long vector)
{
    ...
    if (vector & AFF_WARCRY)
        strcat(buf, " warcry");
  if (vector & AFF_RESTRAIN)
    strcat(buf, " restrain");
  if (vector & AFF_RESTRAIN2)
    strcat(buf, " restrain");
  if (vector & AFF_FIRESTORM)
    strcat(buf, " firestorm");
  if (vector & AFF_STILL)
    strcat(buf, " still");
  if (vector & AFF_BALLAD)
    strcat(buf, " ballad");
  if (vector & AFF_DOME)
    strcat(buf, " dome");
  if (vector & AFF_STANCE)
    strcat(buf, " stance");
  if (vector & AFF_CLEANSE)
    strcat(buf, " cleanse");
  if (vector & AFF_PRAY)
    strcat(buf, " pray");
  if (vector & AFF_STEALTH)
    strcat(buf, " stealth");
  if (vector & AFF_SHROUD)
    strcat(buf, " shroud");
  if (vector & AFF_WOLF)
    strcat(buf, " wolf");
  if (vector & AFF_FOCUS)
    strcat(buf, " focus");
  if (vector & AFF_DIMENSION_VIEW)
    strcat(buf, " Dimen View");
  if (vector & AFF_FEAR)
    strcat(buf, " fear");
  if (vector & AFF_IRONSKIN)
    strcat(buf, " ironskin");
  if (vector & AFF_AWARENESS)
    strcat(buf, " awareness");
  if (vector & AFF_STICKY)
    strcat(buf, " sticky");
  if (vector & AFF_WILLPOWER)
...
char *new_affect_bit_name (long vector)
{
    static char buf[512];
  ...
  if (vector == AFF_STICKY)
    strcat(buf, " sticky");
  if (vector == AFF_MANASHIELD)
    strcat( buf, " mana shield");
  if (vector == AFF_TELE_DISORIENT)
    strcat( buf, " teleport disorientation");
  if (vector == AFF_BLACKJACK)
    strcat( buf, " blackjack");
  if (vector == AFF_BLACKJACK2)
    strcat( buf, " paranoia");
  if (vector == AFF_BANDAGE)
    strcat( buf, " bandage");
  if (vector == AFF_REGENERATION)
    strcat( buf, " regeneration");
  if (vector == AFF_SUPER_INVIS)
...
char *extra_bit_name (int extra_flags)
{
    ...
    if (extra_flags & ITEM_NOBREAK)
        strcat (buf, " no_break");
    if (extra_flags & ITEM_TAINT)
        strcat (buf, " taint");
    if (extra_flags & ITEM_BURNING)
        strcat (buf, " burning");
...
char *act_bit_name (int act_flags)
{
    ...
    if (IS_SET (act_flags, ACT_IS_NPC))
    {
        ...
        if (act_flags & ACT_NOHUNT)
            strcat (buf, " nohunt");
        if (act_flags & ACT_NOQUEST)
            strcat (buf, " noquest");
    }
    else
    {
        ...
        if (act_flags & PLR_AUTOGOLD)
            strcat (buf, " autogold");
        if (act_flags & PLR_HUNGER_FLAG)
            strcat (buf, " hunger flags");
...
char *act2_bit_name (int act2_flags)
{
    ...
    buf[0] = '\0';
    strcat (buf, " player");
    if (act2_flags & PLR_NOIP)
         strcat (buf, " no_ip");
    if (act2_flags & PLR_DEVELOPER)
         strcat (buf, " developer");
    if (act2_flags & PLR_NORP)
         strcat (buf, " no_roleplay");
    if (act2_flags & PLR_MXP)
	 strcat (buf, " mxp");
    if (act2_flags & PLR_NOEXP)
	 strcat (buf, " no_mxp");
    if (act2_flags & PLR_TESTOR)
	 strcat (buf, " testor");
    return (buf[0] != '\0') ? buf + 1 : str_dup("none");
}
...
char *imm_bit_name (int imm_flags)
{
    ...
    if (imm_flags & VULN_SILVER)
        strcat (buf, " silver");
    if (imm_flags & IMM_BOMB)
        strcat (buf, " bomb");
    if (imm_flags & IMM_ARROW)
        strcat (buf, " arrow");
    return (buf[0] != '\0') ? buf + 1 : str_dup("none");
}
...
char *class_bit_name(int class_flags)
{
   static char buf[512];
   buf[0] = '\0';
   if (class_flags & CLASS_MAGE)
     strcat(buf, " mage");
   if (class_flags & CLASS_CLERIC)
     strcat(buf, " cleric");
   if (class_flags & CLASS_ASSASSIN)
     strcat(buf, " assassin");
   if (class_flags & CLASS_WARRIOR)
     strcat(buf, " warrior");
   if (class_flags & CLASS_ARCHER)
     strcat(buf, " archer");
   if (class_flags & CLASS_ROGUE)
     strcat(buf, " rogue");
   if (class_flags & CLASS_ILLUMINATOR)
     strcat(buf, " illuminator");
   if (class_flags & CLASS_FORM)
     strcat(buf, " form_master");
   if (class_flags & CLASS_THIEF)
     strcat(buf, " thief_catcher");
   if (class_flags & CLASS_FORSAKEN)
     strcat(buf, " forsaken");
   if (class_flags & CLASS_DRAGON)
     strcat(buf, " dragon");
   if (class_flags & CLASS_HERBALIST)
     strcat(buf, " herbalist");
   return (buf[0] != '\0') ? buf + 1 : str_dup("none");
}
...
char *wear_bit_name (int wear_flags)
{
    ...
    if (wear_flags & ITEM_WEAR_FLOAT)
        strcat (buf, " float");
    if (wear_flags & ITEM_WEAR_QUIVER)
        strcat (buf, " quiver");
...
int colorstrlen(const char *argument)
{
  ...
  {
    if ( *str == '`' ) //<-- (Replace the { with whatever the color 
    {					//	for your mud is.)
      str++;
      continue;
    }
...
int roomcount( CHAR_DATA *ch )
{
    ...
    /* MAX_EXPLORE is 8192 - see the discussion above */
...
int mobcount( CHAR_DATA *ch )
{
    ...
    for( index = 0; index < MAX_EXPLORE; index++ )
    {
        count += bitcount( ch->pcdata->mobbed[index] );
    }
...
int count_mobarea(int mvnum, AREA_DATA *area)
{
  int i, count = 0;
  ...
  for (i = area->min_vnum;i <= area->max_vnum; i++)
  {
    ...
    {
      CHAR_DATA *mob;
      for (mob = room->people; mob != NULL;
           mob = mob->next_in_room)
           if (IS_NPC(mob) && mob->pIndexData->vnum == mvnum)
             count++;
    }
...
void check_trap( CHAR_DATA *ch, OBJ_DATA *obj )
{
   ...
   if (!str_cmp("archer", class_table[ch->cClass].name))
   {
    return;
   }
   ...
   if (IS_NPC(ch))
    ...
    {
        ch->mana -= number_range( 1, 100 );
    }

    else if ( obj->value[0] == TRAP_TELEPORT )
    {
        act( "$p sends you flying through space!\n\r",
            ch, obj, NULL, TO_CHAR );
